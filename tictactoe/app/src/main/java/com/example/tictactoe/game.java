...
public class game extends AppCompatActivity {
    ...
    private void onCellClicked(int index) {
        ...
        if (playerTurn) {
            board[index] = 1;
            cells[index].setImageResource(R.drawable.o); // Assuming 'o' is the player symbol
            playerTurn = false;
    ...
    private void computerMove() {
        ...
        for (int i = 0; i < board.length; i++) {
            if (board[i] == 0) {
                board[i] = 2;
                cells[i].setImageResource(R.drawable.xsymbol); // Assuming 'x' is the computer symbol
                playerTurn = true;
